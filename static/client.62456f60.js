...
let status_phase;
let status_cycles;
let status_errors; // setTimeout but then wait for the first full frame after that
...
async function monitorServerLoop() {
  let timeout; //console.log('ping start');
  //console.time('ping');

  try {
    timeout = setTimeout(() => {
      timeout = null;
      console.error('ping timed out'); //console.time('ping');

      document.body.classList.add('noserver');
      document.body.classList.remove('rebooting');
    }, PING_TIMEOUT); //console.log('ping fetch');
    //console.timeLog('ping');

    let response = await fetch('/ping'); //console.log('ping fetch done');
    //console.timeLog('ping');
    ...
    if (timeout) {//console.timeLog('ping');
    }
...
function loadContactMessages(project) {
  ...
  let contactus0div = document.querySelector('#contactus0'); //FIXME two more places to tweak the message (turnonjibo and restartstation panels)
...
async function connect_wifi(event) {
  ...
  try {
    ...
    connection_error_message = false; //await fetch(new Request('/connect',{method:'POST',body:formdata}))
...
async function disconnect_wifi() {
  ...
  try {
    ...
    fetch(request); //reset_scroll('#wifi_section');
  } catch (err) {
...
function password_focused(event) {
  let connect_wifi_form = document.querySelector('#connect_wifi_form'); //connect_wifi_form.scrollIntoView(false);
...
async function init() {
  ...
  console.log('md', md); //window.md = md;
  ...
  if (hashParams.project) {
    ...
    if (android_version && android_version >= 9) {
      ...
      fetch(new Request('/debug', {
        method: 'POST',
        body: 'going to attempt browser close on done'
      }));
    }
  }
...
},{"mobile-detect":"../node_modules/mobile-detect/mobile-detect.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
